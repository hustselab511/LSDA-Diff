...
def load_pre_encoded(config, device):
    from dual_unet import DualUNet

    pre_encoded = DualUNet(in_channels=1, out_channels=12)
    pre_encoded.load_state_dict(torch.load(config['pre_encoded']['path']))
    pre_encoded = pre_encoded.to(device)
