...
class ST_MEM_ViT(nn.Module):
    def __init__(self,
        ...
        self.to_patch_embedding = nn.Sequential(Rearrange('b c (n p) -> b c n p', p=patch_size),
                                                ...
                                                nn.LayerNorm(width))
        # (b, c, n, width)
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 2, width)) # (1 ,n+2, width)
        self.sep_embedding = nn.Parameter(torch.randn(width)) # (width)
        self.lead_embeddings = nn.ParameterList(nn.Parameter(torch.randn(width)) # (num_leads, width)
        ...
        # classifier head
        self.head = nn.Identity() if num_classes is None else nn.Linear(width, num_classes) # nn.Identity()直接返回输入张量
    ...
    def forward_encoding(self, series):
        ...
        x = self.to_patch_embedding(series) # (b, c, n, width)
        b, _, n, _ = x.shape
        x = x + self.pos_embedding[:, 1:n + 1, :].unsqueeze(1)

        # lead indicating modules
        sep_embedding = self.sep_embedding[None, None, None, :] # (1, 1, 1, width)
        ...
        x = torch.cat([left_sep, x, right_sep], dim=2) # (b, c, n+2, width)
        lead_embeddings = torch.stack([lead_embedding for lead_embedding in self.lead_embeddings]).unsqueeze(0) # (1, c, width)
        lead_embeddings = lead_embeddings.unsqueeze(2).expand(b, -1, n + 2, -1) # (b, c, n+2, width)
        x = x + lead_embeddings
